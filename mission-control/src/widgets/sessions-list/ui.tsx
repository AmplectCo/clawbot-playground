...
import { $realSessions } from '@/entities/openclaw/model';
...
export const SessionsList = () => {
  const sessions = useStore($realSessions);

  if (sessions.length === 0) {
    return <div className="bg-bg-card border border-border rounded-xl p-6">Loading sessions...</div>;
  }
    ...
    <div className="bg-bg-card border border-border rounded-xl p-6 shadow-lg hover:border-accent-primary transition-all duration-200">
      ...
      <div className="flex flex-col gap-3">
        {sessions.map((session) => (
          <div 
            key={session.id} 
            className="flex justify-between items-center p-4 bg-bg-secondary rounded-lg border-l-4 border-accent-primary"
          >
